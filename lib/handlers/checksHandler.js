...
// Check - Delete
// Required data: id
checksHandlers.delete = function(data, callback) {
    ...
    if (id) {
        _data.read('checks', id, function(err, checkData) {
            if (!err && checkData) {
                                ...
                                _data.read('users', checkData.userPhone, function(err, userData) {
                                    if (!err && userData) {
                                        ...
                                        if (checkPosition > -1) {
                                            ...
                                            callback(500, { Error: 'Could not find the check on the user\'s objet' })
                                        }
                                    } else {
                                        callback(400, { Error: 'Could not find the user who created the check. So check not removed!' });
                                    }
                                });
                            } else
                ...
                callback(400, { Error: 'Could not find specified Check ID' });
        });
